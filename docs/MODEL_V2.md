...
Для третьего режима через Gemini API см. [GenAI](GENAI.md).

Архитектура и развёртывание Model v2 (EfficientNet-B0 384×384).
Модель `model/orientation_model_v2.onnx` полностью обучена на проверенном датасете DocLayNet + CORD (4 093 уникальных страницы) и обеспечивает 98.2% точности на тестовой выборке без привлечения OCR.
В режиме Pure CV инференс выполняется за ~25–35 мс на CPU через ONNX Runtime.
